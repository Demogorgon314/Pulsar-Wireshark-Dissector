...
#define PULSAR_TCP_DEFAULT_RANGE     "6650"
...
static uint32_t get_pulsar_message_len(packet_info *pinfo _U_, tvbuff_t *tvb, int offset,
                                       void *data _U_) {
    return FRAME_SIZE_LEN + (uint32_t) tvb_get_ntohl(tvb, offset);
}

static int dissect_pulsar(tvbuff_t *tvb, packet_info* pinfo, proto_tree* tree, void* data _U_) {
    tcp_dissect_pdus(tvb, pinfo, tree, TRUE, FRAME_SIZE_LEN,
                     get_pulsar_message_len, dissect_pulsar_message,data);
...
void proto_register_pulsar() {
    ...
    proto_pulsar = proto_register_protocol("Pulsar Wireshark Protocol", /* name       */
    ...
    dissector_add_uint_range_with_preference("tcp.port", PULSAR_TCP_DEFAULT_RANGE, pulsar_handle);
